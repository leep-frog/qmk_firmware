...
layer_data_t layer_data[MAX_NUM_LAYERS] = {
    [0 ... MAX_NUM_LAYERS-1] = {},
};
...
layer_state_t layer_state_set_user(layer_state_t state) {
  ...
  for (int i = 0; i < MAX_NUM_LAYERS; i++) {
    ...
    if (current_state != layer_statuses[i]) {
      ...
      if (layer_handlers[i]) {
        layer_handlers[i](current_state, &layer_data[i]);
      }
  ...
  if (LeepHighestLayer == LR_BASE || LeepHighestLayer == LR_ELLA || LeepHighestLayer == LR_SYMB) {
    combo_enable();
  } else {
    combo_disable();
  }
